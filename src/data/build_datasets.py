...
def backup_then_write(path: Path, text: str) -> None:
    ...
    tmp.replace(path)  # ← was: os.replace(tmp, path)
...
def make_dest_paths(
    ...
    stem = src.stem  # e.g., 'nlu'
    dest_yaml = base_dir / f"{stem}{out_suffix}{src.suffix}"  # .yaml

    if src.name == "nlu.yaml":
        jsonl_base = "samples.train"
    elif src.name == "validation.yaml":
        jsonl_base = "samples.validation"
    elif src.name == "test.yaml":
...
def annotate_file(
    ...
    """
    Map legacy intents to 4 parents and write:
      - YAML with parent-as-intent, subintent (when allowed), legacy_intent, literal examples block
      - JSONL with label + subtype field per parent
    Non-destructive if out_dir/suffix provided.
    """
    ...
    nlu_items = doc.get("nlu") or []
    if not isinstance(nlu_items, list):
        print(f"[skip] {path.name} nlu section not a list")
        return
    ...
    for item in nlu_items:
        ...
        # Idempotent: prefer legacy_intent if present
        legacy_intent = str(
        ...
        # Build YAML block (order: intent -> subintent -> legacy_intent -> examples)
    ...
    if dest_yaml.resolve() == path.resolve():
